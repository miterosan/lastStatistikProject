...
aida::get_samples_regression_noninformative(X,y , 10000)
```
...
```{r}
aida::summarize_sample_vector(data_MC_excerpt, c("shape", "block"))
```
...
Long story short: All the credible intervals for the respective slopes contain a range from negative to positive values, so also include 0. Thus we can reject both hypotheses. There is significant evidence that gender is not a main effect and that no interaction between gender and condition exists.
